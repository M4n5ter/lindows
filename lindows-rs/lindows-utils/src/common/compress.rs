...
thread_local! {
    static COMPRESSOR: RefCell<io::Result<Compressor<'static>>> = RefCell::new(Compressor::new(crate::common::config::COMPRESS_LEVEL));
        ...
        if let Ok(mut c) = c.try_borrow_mut() {
            match &mut *c {
                Ok(c) => match c.compress(data) {
                    Ok(res) => out = res,
                    Err(err) => {
                        log::debug!("Failed to compress: {}", err);
                    }
                },
                Err(err) => {
                    log::debug!("Failed to get compressor: {}", err);
                }
            }
        }
    });
            ...
            match &mut *d {
                Ok(d) => {
                    ...
                    match d.decompress(data, n) {
                        Ok(res) => out = res,
                        Err(err) => {
                            log::debug!("Failed to decompress: {}", err);
                        }
                    }
                }
                Err(err) => {
                    log::debug!("Failed to get decompressor: {}", err);
                }
            }
        }
    });
