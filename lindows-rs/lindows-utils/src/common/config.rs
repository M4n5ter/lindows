...
#[inline]
pub fn store_path<T: serde::Serialize>(path: PathBuf, cfg: T) -> crate::common::ResultType<()> {
        ...
        if let Ok(peers) = Config::path(PEERS).read_dir() {
                ...
                .collect::<Result<Vec<_>, _>>()
            {
                let mut peers: Vec<_> = peers
                    ...
                    .map(|(id, p)| {
                        let t = crate::common::get_modified_time(p);
...
impl LanPeers {
    ...
    pub fn modify_time() -> crate::common::ResultType<u64> {
